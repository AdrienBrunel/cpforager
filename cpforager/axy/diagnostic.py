...
def full_diagnostic(self, fig_dir, file_id, plot_params, fast=False):   
    ...
    circ_mean_pitch = self.circ_mean_pitch
    circ_mean_roll = self.circ_mean_roll
    df_tdr = self.df_tdr
    df_gps = self.df_gps
    ...
    infos.append("Circular mean pitch = %.3f" % circ_mean_pitch)
    infos.append("Circular mean roll = %.3f" % circ_mean_roll)
