...
def full_diagnostic(self, fig_dir=str, file_id=str, plot_params=dict):   
    ...
    infos.append("Number of GPS measures = %d" % n_gps_df)
    ...
    infos.append("Median odba = %.1f" % median_odba)
    infos.append("Median odba_f = %.1f" % median_odba_f)
    ...
    diagnostic.plot_ts_twinx(ax, df.iloc[0:1000], plot_params, "odba", "Overall Dynamic Body Acceleration [Zoom]", "ODBA [g]", scatter=False)
